...
| **65** | **Proposition 6 numerical confirmation** (horizon tightness; the ICLR-draft central-claim figure) | the exact Prop-6 construction on a controlled multi-channel latent: $\epsilon$-approx-equiv orbit-variation $=\epsilon e^{\lambda_j T}$ to rel-err **$10^{-14}$** (tight lower bound); exact-equiv flat to $10^{-16}$ at all $T$; conserved channels ($\lambda\le0$) infinite-horizon; certified horizon **linear in $\log(1/\epsilon)$, slope $1/\lambda$ ($R^2{=}1.000$)**. Confirms the proved construction (trained-model degradation is Exp 8) | `test_step65.py` (Prop-6 lower-bound exactness, 1 test) | this batch (3 seeds) |
