...
## [2026-06-10] step99(Exp 31,E16)— 真机数据监控:实测列推翻切空间数字;"交叉验证的审计才是可部署对象"在 1B 尺度成为必需品

**全量(20 条真实 DROID episodes,2400 监控步)**:invalid=1.00 @ 全部 k(含 k=1);一步中位 0.632 ≈ 原生
连续潜距 0.681(预测器优于复制基线 0.748,价值在能量排序而非度量预报——franka 能量序 random 0.471 ≫ true
0.426 验证管线正确);staleness 误差 0.625→0.787/8 步,**对数斜率 0.033(逐窗中位 0.028)≪ 认证 λ1=0.178**。
预注册分支 G8-E **按注册原样失败**;预注册子分类规则判 **bias**。机制:部署误差从表征原生步距起步,
**从未进入线性化邻域**——Prop 7 偏差简并的基础模型尺度形态,切空间谱的辖区与监控工作点不重叠。**头条:
纯谱审计会对旗舰基础世界模型过度承诺 T1≈9;E13 协议的实测交叉验证列正确推翻了它——交叉验证的审计
(而非裸谱)才是可部署对象。** 两条可迁移教训入文:阈值是表征相对的(此 token 几何原生每步 ~0.68,
DMC 的 θ=0.2 约定不迁移);增长率只在线性化邻域内定价。措辞红线全程遵守:"real-robot data, offline
monitoring"(被动监控 ⇒ 离线重放是忠实实例化)。三联判别探针(连续距/复制基线/能量排序)先于解释、
排除协议 bug;条件 gate 规格在读证书前冻结。E16 折叠:ICLR(摘要 override 句、(vi) 三家族、E16 段、
附录 D 全协议、等量削减)+ 长版 Exp 30–31 全节。tests/test_step98_99 3/3;全套 226 绿。
**至此"像素/真机"弱点的最终状态**:像素家族部署 cell(step97 LeWM)+ 1B 基础模型审计 + 真机数据离线监控
(step98/99 V-JEPA 2-AC + DROID)全部落地;不可达的只剩"真机闭环控制"(无硬件,诚实 out-of-scope)。

## [2026-06-10] 联合通读 R1–R8(E16 终态稿)— 7 轮修复,2 个硬伤,全部入库

与用户逐节通读投稿稿(checklist v2)。**抓到的硬伤**:① $\hat z_T$ 未定义(压缩事故,§2 滚动定义还原);
② Noether hinge 的 "exactly" 双向超claim(与 Prop 5 自己的 scope 注矛盾)——四处统一改为单向措辞,新增防线句
"the one direction proved is the one the certificate uses"。**其余修复**:摘要第五拍拆句+deployable-object 还魂;
接缝段 E13–E16 + 1B 见证句;Lemma 2 行内 sketch 升级为可验证等式链((A4) 步显式);Prop 7 的 o(t)→(1+o(1));
Prop 6′ 指针独立成段;"(the converse)"→"(Lemma 2)";Liouville 锚展开复原;E12 尾注范围记号越界修正
(step85–88 误圈 86/87);统一段 E13–E16 + load-bearing-by-E16;invalid 4%→4.0–4.4%;E15 巨段拆分;
BRo/UWM arXiv id 内联;结论 E13–E16;双 HR 清理。布局决策(用户签):E2 两图留附录 A,step65 保正文。
226 测试;PDF 重建待箱子回线(md 已全推)。

## [2026-06-11] arXiv 占坑包就绪(用户决策:先占坑)

排雷(e2cnn 事故 paper2 侧不暴露)→ 复盘 → 用户拍板占坑。箱回线:三 PDF 带通读修复重建(References p9@81%)。
`build_arxiv.py`:非匿名变体(\iclrfinalcopy + 实名/邮箱 + pdfauthor 元数据 + sty 页眉 "Published as"→"Preprint."
...
## [2026-06-11] 加 n 行动 P1 落地:摆环 n=5→15,机制确认(Fisher p=0.002,Spearman ρ=0.96)

用户拍板"效应弱处加 n"。预注册先行(flags 由旧 5 籽标定、新 10 籽检验,spec 时间戳先于数据):膨胀≥1.5×、
赢≥0.10。**结果:2×2 = [膨胀&赢 9 | 膨胀&不赢 1 | 不膨胀&赢 0 | 不膨胀&不赢 5];Fisher 单侧 p=0.002;
Spearman ρ=0.956(p=2.6e-8)——反机制格子空,唯一例外 seed 8 恰在 flag 边界(1.64×,+0.033)。判定:
MECHANISM CONFIRMED(按预注册规则)。** "2/5 轶事"升格为带剂量-响应曲线的机制律——E12 最薄承重点变最硬。
新 10 籽 box 实跑 ~40 分钟(我此前估 3–5h,误差 5–7×,记入估时校准)。产物:step88_ring_frontier_n15.json
(原 5 籽正典保留);折叠:ICLR E12 句 + 长版 §5.20 段;tests/test_step88_n15(统计量重推导)过。
并行中:step93 ×10(box venv 被外部翻新,dm_control/模型缺失——补装重启)、step99 ×40(GPU,进行中)。

## [2026-06-11] 加 n 行动 P3 落地:step99 DROID 20→40 集——E16 数字稳如磐石

n=40(数据集上限内)vs n=20:一步中位 0.632→0.629、斜率 0.033→0.030(逐窗 0.028→0.027)、原生步距
0.681→0.680、复制基线 0.748→0.742;判定不变(G8-E 如实失败、bias 子类)。**主文圆整引用(0.63/0.68/0.03)
零改动**——只更新附录 D 与长版的精确小数 + 集数。n40 升正典,n20 存档(step99_droid_monitor_n20.json)。
工程注:box venv 被外部翻新(dm_control 丢失、mujoco 漂到 3.9.0 与 dm_control 不配对,flex_bandwidth
AttributeError)——按 Mac 验证对钉回 mujoco==3.3.2 + dm_control==1.0.28,step93 第三次重启。

## [2026-06-11] 加 n 行动 P2 落地:step93 3→10 episodes/cadence——膝点与范围律原样复现,锚点更优

工程三连坑(模型文件丢→dm_control 丢→mujoco 3.9.0 配对错)逐一排掉后第三次发射成功。n=10 曲线:
anchor 986.1(范围 976.7–996.3,**与官方带 977–983 重叠**——比 n=3 的 993.5–996.3 更佳);k=2 即 892.7
(90%)、k=4 650、k=6 569、k=8 486、k=12 347、k=24 191——膝点 k≈2 原样,INCONCLUSIVE 判定与
Prop 11 的 H(θ*)≈2 引用不变。n=10 升正典、n=3 存档。**加 n 三连全部收官:P1 机制确认(ρ=0.96)、
P2 膝点复现+锚点改良、P3 数字稳定——零 gate 改动,三处全是统计增厚。**

## [2026-06-11] 过夜加厚链总收官 + 84-cell 扩张推翻种子图轴(今日主发现)

**R2(step89b)**:全 zoo 审计 69/69 新 cells(34 任务宇宙、自执行纳入规则、11 自定义变体被规则排除、
fish-swim-3 一次下载损坏重试)。**84-cell 终图:弃权 42/84(稳 13/偏 29)、膨胀 42。种子图"强膨胀⇒校准"
不成立**——带内 λ₁ 横跨 0.01–0.39;真轴 = 测量视界由增长还是偏差设定:偏差主导(med≤3)24/42(dog/
humanoid/quadruped 整块,E16 机制 zoo 尺度),增长区(med≥5)15 格**中位 ratio 0.95、带内 10/15**。
cut 敏感性 0.49/0.65/0.89(≤1/≤2/≤3)全披露;重分层标注描述性,冻结量按注册原样报。对照:walker-run-2
(已证明展平域)0.95,同任务籽 1/3 = 0.06/0.35——低 ratio 非管线伪影。**修正后读法全尺度统一:谱定价增长、
measured 列检验水平、审计 = 合取。** 双版论文 + 摘要 + 贡献(vi) + hero 图说折叠;rebuttal A12(自家扩张
推翻自家分类学 = 可证伪性演示);test_step89b 8 项(翻译器含 trailing-LN 碰撞案例 + 全图统计再推导)。

**R3/R4 重生**(reset --hard 事故后):step94 n=100(G1a 3/3、**G2 升 3/3**、G1b 仍带边 INCONCLUSIVE;
ratio 与 n=20 逐位同)、step96 n=100(**G3 降 1/2**:finger-spin-2 invalid 5.6% 压线 0.6pp,按注册记
INCONCLUSIVE not PASS;G4 hopper 8.0 带内、G5 1.04)、step95 加厚(41 样本:L96 11.85@W400 **过自身
收敛检**、walker 49.21@W200 vs 20.9@W120 窗口依赖、cheetah 18.74)——三者与被冲日志逐位一致,确定性
协议背书。test_step96 改写为 as-registered INCONCLUSIVE 台账。

**R1(step85 ×17 新籽 → n=20)**:**20/20 全胜,margin +0.41–+0.61,中位 +0.53,bootstrap 95% CI
[0.484,0.542];违规率中位 10% vs 63%(范围 3–19/53–70);通胀 2.9–5.2×(中位 3.49)。** E12 主数字
从 3 籽轶事升格为带 CI 的 20 籽律;控制组(再校准/自适应/UQ)保持 3 籽如实标注。

**工程事故簿(全闭环)**:① 我在 R3/R4 工件落盘后 `git reset --hard` 冲掉 n=100/加厚 JSON——重跑全部
逐位复现;新规:**工件落盘即 scp 离箱,链终前箱上零 git 写操作**。② Rosetta 失效(arch -x86_64 都过不了)
→ pandoc/tectonic 换 arm64 原生(~/.local/bin)。③ 两次 ssh 255 真凶 = pkill -f 模式自匹配杀掉自己的
远端 shell——括号免疫法 `pkill -f "x_[y]z"` 入册。④ numpy 2 删 in1d、dm_control quadruped 还在用 →
垫片 + reset 纳入 try + 工程失败独立台账。⑤ step98 五籽被 OOM-killer 静默杀于 seed 3 → 脚本改逐籽落盘
+ 可续跑 + gc,重发中(前三籽 0.1800/0.1767/0.1780,散布 <2%)。

**9 页红线保卫战**:E13 扩张段使正文溢出 0.34 页 → 三轮外科压缩(E13 段 30%、贡献(vi)、E15/96 合句、
κ 句、limitations 两弹、Conclusion 并句、独立显示块并段、图宽 55/58/65/56)→ References 干净开 p10,
正文恰 9 页。**249 测试全绿。**

**新备用机纳编**:hongbowang@100.119.233.53(Intel x86_64,16 核,16GB,macOS 26.5)——uv + repo +
venv(torch 2.2.2 是 Intel mac 终版,numpy 钉 1.26.4)+ 冒烟 9/9 过。定位:paper2/3 CPU 小实验;
...
## [2026-06-12] step100 桥实验:INCONCLUSIVE-BY-LEVEL-DOMINANCE(按注册如实记)+ 两个真收获

30 cells(3 臂 ×10 籽,5000 epochs,box 四道 ~12 分钟):G-EQ 最强形式过(eq 缺陷**字面 0.0** 十籽,
dense 1.1–1.4);但 28/30 证书弃权(CI 跨零)、30/30 测量中位=1——从零模型未达增长定价开赛线,
结构 vs 稠密之争未能开场。判定按注册第三分支如实记。**真收获**:① **dmcontrol walker 是严格 S₂ 对称
的**(双腿交换缺陷中位 4.75e-16——新的可引用测量事实,G-SYM 探针);② 首个 dmcontrol 任务上的
S₂-等变 WM(置换绑定 float-exact);③ 比例尺教训:桥要有意义需 TD-MPC2 级训练(GPU-天),九月再决
是否升级。工程注:e400 诊断版存档;Mac 后台 QoS 节流嫌疑(step82 四道 3h 仅 36min CPU + 两道无声死)
→ 长跑全部迁 box,Mac 只做交互级。step82 ×10 已在 box 四道重发。

## [2026-06-12] step82 ×10 收官(完备主义批次最后一项)+ 诚实更正

四道(籽 0-2/3-5/6-7/8-9)全收,跨批稳定:CatMap(learned) tightness 1.173/1.213/1.256/1.196 →
**n=10 区间 1.17–1.26×**;Henon(true) 3.16–3.20×(~3× 保守);soundness 4/4 批全 1.0;Henon cone-margin
负(弃权)。**诚实更正**:论文原引 "1.17×(3 seeds)" 实为 n=3 lane-A 地板,n=10 真实区间 1.17–1.26×——
