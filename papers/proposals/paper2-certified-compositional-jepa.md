...
**§2.1 Resolution-stratified certificate (the 推背图 refinement — promote $L$ from a scalar to a spectrum).** The single Lipschitz constant $L$ is a coarse summary; the predictor's Jacobian has a *spectrum* $\{e^{\lambda_j}\}$ along latent eigen-/isotypic directions. Decompose the latent into channels (e.g. the $\ell$-isotypic blocks: $\ell{=}0$ invariants vs $\ell{\ge}1$ pose/detail). Then long-horizon predictability is **channel-dependent**:
$$
\text{a channel with rate }\lambda_j\text{ is certified to horizon } T_j(\epsilon)\ \sim\ \tfrac{1}{\lambda_j}\,\log\tfrac{1}{\epsilon}\quad(\lambda_j>0),\qquad T_j=\infty\ (\lambda_j\le 0).
$$
So the **coarse / invariant / slow** content (small or non-positive $\lambda_j$ — typically the $\ell{=}0$ block and any conserved quantity) is certified **arbitrarily far**, while **fine / expansive** detail ($\lambda_j>0$) is certified only to a $\log(1/\epsilon)$ horizon. Equivalently, a **horizon × resolution trade-off**: $\text{horizon}\times\text{(demanded resolution)}\lesssim$ const set by the symmetry/Lyapunov spectrum. This is the formal content of the folk observation that **ultra-long-range forecasts must be coarse** (almanacs predict eclipses for millennia; weather dies at ~2 weeks; the 推背图-style texts only ever assert regime-level change, never detail — structurally the *correct* move). It also **bridges the two axes**: by a Noether-type link, the *group* (equivariance, §2) supplies the *conserved/slow* directions (the small-$\lambda$ channels), so the symmetry that gives across-orbit generalization is the *same* structure that gives long-horizon predictability — **when the group-invariants are dynamically conserved** (a nontrivial, testable condition, not automatic — the open hinge). *(Speculative extension — possible standalone "structure theory of long-horizon predictability" / paper3: what is ultra-long predictable = the coarse-invariant submanifold; horizon×resolution bounded by the group + Lyapunov spectrum; used to adjudicate celestial mechanics / climate / weather / classical forecasting systems under one criterion.)*
