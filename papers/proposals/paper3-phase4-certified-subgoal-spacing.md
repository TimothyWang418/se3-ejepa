...
**Fraction semantics (pinned; v1.1, see §3.3):** fractions are **episode-level** and apply to the
**encoder-training corpus** (WeakPolicy interaction episodes — the $\kappa$-uniform protocol)
$\{1\%,5\%,10\%,30\%,100\%\}\approx\{2,10,20,60,200\}$ episodes — the thesis-relevant quantity is
representation sample-efficiency on interaction data. The probe's own training budget is **fixed and
identical across all cells** (held-out episodes), so probe sample-efficiency is never the thing
measured. The human 206-episode set replicates the $\kappa{=}0$ row as a robustness check.
...
2. **PushT-dyn($\kappa$)**: damping/momentum parameterization of the same task — dial $\kappa$ to
   ...
   **Demo regeneration (the knob's hidden chain-cost — resolved by W1 recon):** changing the physics
   invalidates the static human demos — each $\kappa$ generates its own sets via the v1.1 protocol
   (§3.3): WeakPolicy for the encoder corpus + **oracle-CEM on the true env** (state-reset MPC;
   `reset(options={'state':...})` and the CEM/iCEM/MPPI solver suite are vendored in `third_party/`,
   W1-verified). The knob itself **pre-exists as the env's `damping` constructor parameter**
   (default `space.damping=0` = the quasi-static anchor) — zero environment engineering.
   **Fairness rule (pre-registered):** all hyperparameters tuned at $\kappa{=}0$
...
### 3.3 Data — registered protocol v1.1 (amended 2026-06-10 post-W1-recon, **before any training run**)

W1 recon (`docs/specs/2026-06-10-p4-w1-recon.md`) forced one clean change, registered here while no
data has been seen: the $\kappa$ sweep forbids human demos (they exist only at default physics), so
the protocol is **uniform across $\kappa$**:

- **Encoder-training corpus** = WeakPolicy interaction episodes (the vendored collection policy);
  200-episode budget per $(\kappa,\text{seed})$. **C2's fractions apply to this corpus** —
  $\{1\%,5\%,10\%,30\%,100\%\}\approx\{2,10,20,60,200\}$ episodes. The claim thereby measures
  world-model representation sample-efficiency on *interaction data* (thesis-aligned: a JEPA is
  self-supervised on interaction, not curated demos); R-aug trains on the same corpus $\pm$ rotation
  augmentation.
- **Successful-demonstration set** (trains $G$; C4's budget axis) = **oracle-CEM rollouts on the
  true env** (state-reset MPC), 200 successful episodes per $\kappa$ — one generator across all
  $\kappa$, no human-vs-scripted confound.
- **Human lerobot/pusht set** (verified: 206 episodes, 96×96, 10 fps) = $\kappa{=}0$ **robustness
  check only** (one replication row of E-P4.1/E-P4.4; ecosystem comparability with LeWM/FF-JEPA).
- **Wedge-restricted orientations** (paper2 Exp 7/9/step51 protocol) for OOD cells — applied to both
  corpora by filtering initial block orientations.
...
| **E-P4.4** | C4 | Closed-loop factorial: cells per §3.1 priority $\times$ {in-dist, OOD-orientation} $\times$ **successful-demo budget for $G$** $\{100\%,10\%\}$ (oracle-CEM set, §3.3); success @ $t\in\{25,75,150\}$; 3 seeds $\times$ ~50 episodes/cell. |
