...
![The certificate at a glance. **Left:** an equivariant model certifies the *entire* generated monoid $\langle S\rangle$ from $k$ generator checks (Lemma 1), up to a horizon ceiling set by the predictor spectrum (Theorem B, tight by Proposition 6); a non-equivariant model of any size certifies only an interpolation tube. **Right:** the horizon-resolution law $T_j(\epsilon)\sim\log(1/\epsilon)/\lambda_j$ — conserved/invariant channels ($\lambda\le0$) are certified to all horizons, chaotic ones shrink with demanded resolution.](figures/hero_certified_region.png){width=68%}
