...
Because the spectrum is faithful, the certificate **acts, a priori**: under a fixed sensing budget a $c\times$-inflated certificate provably needs $c\times$ the budget (Proposition 9), and the equivariant certificate meets a budget its inflated dense counterpart cannot — with zero calibration data. The same read-out, unchanged, **audits public pretrained world models training-free**: official TD-MPC2 checkpoints land on the certificate's own scope taxonomy — calibrated where strongly expansive (ratio $0.94$–$1.02$), optimistic where weakly expansive, correctly abstaining where contracting — and a deployed sensing-budget monitor then replicates that map **cell-by-cell, out-of-sample, at zero new estimation**. Across the official $1$M–$317$M multitask ladder, calibration does **not** improve with parameters — and on V-JEPA 2-AC ($1$B, real robot data) the audit's measured cross-check correctly **overrides** an over-promising tangent spectrum: the cross-validated audit is the deployable object. Scale buys interpolation, not a calibrated horizon.
...
front of it, **whether the model can be trusted, and for how many steps.** Scaling lowers the average and widens the
interpolation region; it certifies nothing about a *particular* unseen configuration and says nothing about the
*horizon* — the steps before compounding error makes the rollout useless.
...
(isotypic blocks: Schur placement, zero cross-block leakage, machine precision), attained to four digits under
controlled obliqueness with the predicted $\log\kappa/\lambda$ horizon haircut; on real loops it is *computable
...
**(E16) At foundation-model scale the cross-check column is load-bearing (`step98`–`step99`).** Audited along its own
energy-landscape rollout, **V-JEPA 2-AC** ($1$B-encoder, official checkpoint, authors' code; $d{=}360{,}448$ — the
largest loop audited here) reads **expansive** ($\lambda_1{=}0.178$, two-seed agreement $1.8\%$). But on $20$ real
DROID episodes the deployment error *starts* at the representation's native step motion ($0.63$ one-step median vs
$0.68$ consecutive-latent distance; the predictor beats a copy-of-last-read baseline, $0.63$ vs $0.75$) and grows at
log-slope $0.03\ll\lambda_1$ — it never enters the linearization neighborhood; the pre-registered pricing gate
fails *as registered*, and the **measured column overrides the tangent number**: Proposition 7's bias degeneracy at
$1$B scale. A spectrum-only audit would have over-promised $T_1{\approx}9$; **the cross-validated audit is the
deployable object** — and thresholds are representation-relative, while rates price only errors inside the
linearization neighborhood (full protocol and engineering disclosures in Appendix D).




**What does structure buy, if the read-out audits any smooth model?** The law applies to any $C^1$ latent map — hence E13. What it cannot supply there is *trust in the number*: a dense spectrum can be silently wrong while predictions stay good (E2), so a generic certificate must be cross-validated against held-out divergence — E13's per-model check. Structure removes that requirement where it holds (E2 $\Rightarrow$ E12's zero-calibration action), exclusively so (Lemma 2). **The audit is universal; the a-priori guarantee is structure's.**
...
- **Prefactor: computable and measured — worst-case vs typical disclosed.** $c_j=1/\sin\theta_j$ (Prop. 6′) is
  ...
  $\kappa_1$ carries a heavy near-tangency tail (median $\sim20$, max $\sim10^2$–$10^3$): the measured calibration
  ($0.83$–$1.02$) reflects *typical*, not adversarial, defect alignment — an adversarially-aligned defect could
  spend the $\log\kappa_1/\lambda_1$ haircut, and the Lorenz-96 angle estimator fails its own convergence check
  (disclosed).
...
- **The horizon law lifts to learned chaotic models and is vacuous on near-neutral dynamics (scope, not a bug).**
  ...
  contact simulator.
- **Demonstrated downstream value is budgeted efficiency, not safety.** A separate test of the certified horizon as
  a *catastrophe-avoidance* cadence was **inconclusive** at our scale (escape was re-observation-interval-invariant —
  control-quality-limited); the certificate's *necessity* for safety is open.
