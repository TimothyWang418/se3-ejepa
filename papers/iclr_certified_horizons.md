...
**(E13) The certificate reads a *public pretrained* world model — training-free.** We rebuild the latent-dynamics slices of the official **TD-MPC2** checkpoints (5M single-task DMC models, 3 official seeds each; MIT) and run the *unchanged* certificate machinery on the policy-prior closed loop $g(z)=d(z,\tanh\mu_\pi(z))$ — no training, and no environment access on the certified side. On **walker-walk** the latent loop is expansive ($\lambda_1=0.25$–$0.30$, tight bootstrap CIs, $3/3$ seeds) and the certificate reproduces the paper's two-regime pattern on a model we did not train: optimistic at tight $\epsilon$ (ratio measured/certified $0.08$–$0.20$ at $\epsilon{=}0.05$, the Proposition 8 $\delta$-bias) and **calibrated at the coarse resolution — ratio $0.94/0.95/1.02$ at $\epsilon{=}0.2$, $3/3$ seeds** (measured = latent open-loop vs. the encoder run along the true dm\_control rollout). On **acrobot-swingup** the SimNorm-compact latent loop is *contracting* ($\lambda_1\approx-0.04$) and the certificate **abstains — correctly, by the scope theorem (Proposition 7)**: the residual divergence there (median $14$–$45$ steps) is bias-driven, not amplification-driven, outside what a Lyapunov certificate covers; issuing one would over-claim. The SimNorm structural zero-directions appear as a strongly-negative spectral band ($128$–$270$ directions), reported, not hidden; the certified scope is the prior loop, not the MPPI planner. Closest prior work computes Lyapunov exponents of the *true environment* under RL policies (arXiv:2410.10674); a Jacobian certificate of the *learned latent map* of a public world model, cross-validated against its true-environment divergence, is new. (`step89`.)
