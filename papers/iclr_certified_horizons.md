...
`step96`–`step97` complete the map: the stable-abstain cells deploy as **free monitoring** ($93$–$94\%$ of windows never cross; invalid $4.0$–$4.4\%$; recall $1.00$), the bias-driven cells land inside the $\times1.5$ band of bench (hopper-1: $7.0$ vs $5.5$ — the clock the certificate rightly refused to price) or buy **zero sensing savings** on the architecturally disjoint **pixel family** (LeWM/PushT: usable cadence $1$, alarm channel flooded at every $k\ge2$, fault detection inseparable from drift — all stated before the run), and a further replication cell lands $1.04$ vs $0.95$ (finger-spin-1).
