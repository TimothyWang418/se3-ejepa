...
1. **A tight certified horizon (§3.2).** For an equivariant model the $T$-step rollout error is orbit-constant
   ...
   (Proposition 7) says *when* the measured spectrum governs a *learned* model's horizon, and a continuity bound
   (Proposition 8) makes that lift rigorous. The staircase then transfers to learned models of genuinely chaotic
   systems (E2: exponents matched to $1$–$12\%$); the near-neutral PushT interior is the predicted degenerate case.
...
**What is new.** A prior-art sweep finds the corner *empty*: no work combines equivariance with a certified, two-sided spectral horizon; the closest guarantee-bearing neighbours each miss the intersection [@conradie2026trustkoopman; @lillemark2026flowm; @mo2026symmetry] (§5). Beyond the tight two-sided horizon — its matching lower bound seeded by the *equivariance residual*, not an initial-condition error (Proposition 6) — and its rigorous lift to *learned* chaotic models where shadowing cannot transfer the exponent (Props. 7–8, §3.2), the new results are: **(i)** the structure-vs-recurrence separation at $40$-D (E2), the recurrent failure forced by the conditional-Lyapunov condition; **(ii)** Theorem B′ turning the certificate from *measured* into *literal* — sound a priori, tight on uniform hyperbolicity, self-abstaining elsewhere; **(iii)** actionability **a priori, with zero calibration data** (E12; Proposition 9); and **(iv)** a training-free trustworthiness audit of public pretrained world models — the TD-MPC2 zoo at **$84$ cells**, LeWM, V-JEPA 2-AC at $1$B (E13–E16) — whose $84$-cell expansion **overturns the seed map's own axis** (calibration tracks growth-set vs.\ bias-set horizons, not $\lambda_1$) and that a deployed monitor replicates **cell-by-cell, out-of-sample, every taxonomy cell type** (E15), its decision boundary formalized as a regret decomposition (Proposition 11). These assemble (Algorithm 1) into the single certificate above; the classical pillars we build on — the Lyapunov/NWP horizon law, invariant decision theory, the $e^{\lambda T}$ growth — are credited in §3 and §5.
...
exponent off its Jacobian field alone. On **uniformly-hyperbolic** dynamics it is *tight*: cat-map certified exponent exact on the true map, $1.17$–$1.26\times$ on a *learned* net ($10$ seeds, all sound), $1.06\times$ on a nonlinear Anosov perturbation. On **non-uniformly-hyperbolic** Hénon a single metric is provably limited — sound but $\sim\!3\times$ conservative — and the certificate's own cone-margin diagnostic turns negative: it **abstains** rather than over-claim. A tight a-priori horizon is achievable *exactly* in the uniformly-hyperbolic regime, and the certificate **self-diagnoses** its regime (`step82`).
...
- **The Noether hinge: forward direction proved, hypotheses measured, emergence open.** Proposition 5 assumes a $G$-invariant
  ...
  not *emergence* in un-constructed dynamics; open.
...
- **Scale and modality.** All experiments are $1$–$2$-GPU. Exact flatness transfers across modalities (state,
  ...
  architecture-agnostic (the anti-collapse JEPA latent, not equivariance), orthogonal to the certificate.

---

## 7. Conclusion

An equivariant world model can certify, a priori, which situations it will handle and **for how many steps** — the horizon tight, its unbounded edge anchored by the conserved subspace, exclusive to structure. Faithful, hence actionable: budgeting sensing (E12), auditing public world models (E13–E16), pricing a deployed monitor (E15). *Scale buys interpolation; structure buys a certified horizon.*
