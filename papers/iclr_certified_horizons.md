...
**(E16) At foundation-model scale the cross-check column is load-bearing (`step98`–`step99`).** **V-JEPA 2-AC** ($1$B encoder, official checkpoint, authors' code; $d{=}360{,}448$ — the largest loop audited here) reads **expansive** ($\lambda_1{=}0.178$, two seeds agreeing to $1.8\%$) — yet on $20$ real DROID episodes the deployment error *starts* at the representation's native step motion ($0.63$ one-step vs $0.68$ consecutive-latent distance) and grows at log-slope $0.03\ll\lambda_1$: it never enters the linearization neighborhood, and the pre-registered pricing gate **fails as registered** (measured sub-class: bias — Proposition 7's degeneracy at $1$B scale). A spectrum-only audit would over-promise $T_1{\approx}9$; **the cross-validated audit is the deployable object**. Thresholds are representation-relative; rates price only errors inside the linearization neighborhood (full protocol, baselines, and engineering disclosures: Appendix D).
