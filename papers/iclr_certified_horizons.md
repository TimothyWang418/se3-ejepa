...
$e^{\lambda T}$ — a single-step tie between augmentation and equivariance **must break over horizon**. **The
prefactor is the splitting conditioning (Proposition 6′, Appendix B):** $c_j=1/\sin\theta_j$ — exactly $1$ on
orthogonal invariant splittings (isotypic blocks: Schur placement and zero cross-block leakage measured at machine
precision), attained to four digits under controlled obliqueness with the predicted $\log\kappa/\lambda$ horizon
haircut, and *computable from the audited Jacobian field* on real loops, where it is honestly a distribution
(median $\kappa_1\approx17$–$21$, heavy near-tangency tail) while measured calibration stays $0.83$–$1.02$
(`step65b`, `step95`).
...
$\lambda_1^{(T)}=\tfrac1T\mathbb E\log\lVert D\phi^T\rVert$ is locally Lipschitz over a finite orbit: a learned
$\hat\phi$ with one-step fidelity $\delta$ recovers the staircase slope up to $O(\delta)$ plus the finite-$T$
truncation, $T$-uniform under a dominated splitting [@bochi2005lyapunov]. The match is **falsifiable** — the exponent
must tighten as one-step error drops — confirmed in E2 (Rössler: $44\%\to8\%$).
