...
**The full-zoo expansion ($84$ cells) overturns the seed map's axis (`step89b`).** Re-running the unchanged protocol on **every remaining public single-task dmcontrol checkpoint** — universe fixed by a pre-registered, *self-executing* inclusion rule (a task enters iff stock dm\_control loads it; $11$ custom variants excluded by the rule, listed in the artifact), $23$ further tasks $\times$ $3$ seeds, $69/69$ cells — gives, as frozen: **$42/84$ abstain** (stable $13$, bias $29$), $42$ expansive with measured/certified median $0.50$ at $\lambda_1\ge0.25$ and $0.33$ below it. **The seed map's "calibrated where strongly expansive" does not survive the zoo**: in-band cells span $\lambda_1=0.01$–$0.39$. What predicts calibration is *which quantity sets the measured horizon*. Where the native one-step residual already sits at the threshold (measured median $\le3$ steps: $24$ of $42$ expansive cells — dog, humanoid, quadruped), the certificate prices growth the rollout never exhibits — **E16's mechanism at zoo scale**, Proposition 7's degeneracy direction now measured in bulk. Where the horizon is long enough to be growth-set (median $\ge5$: $15$ cells), the certificate is **calibrated**: ratio median $0.95$, $10/15$ in $[2/3,3/2]$, across five domains including two absent from the seed map (cup-catch $1.15/1.44$, cartpole-balance-sparse $1.14$). The re-stratification is descriptive, not pre-registered — cut sensitivity disclosed (growth-side median $0.49/0.65/0.89$ at cuts $\le1/\le2/\le3$); the frozen quantities are the protocol and the fractions above. The corrected reading is uniform from toy to zoo to $1$B: **the spectrum prices error *growth*; the measured column tests error *level*; the audit is their conjunction.**
...
- **Prefactor: computable and measured — worst-case vs typical disclosed.** $c_j=1/\sin\theta_j$ (Prop. 6′) is
  ...
  $49.2$ at $W{=}200$, each passing the same stability check — the check is necessary, not sufficient; max $\sim10^2$):
  the measured calibration ($0.83$–$1.02$) reflects *typical*, not adversarial, defect alignment — an
  adversarially-aligned defect could spend the $\log\kappa_1/\lambda_1$ haircut. At $W{=}400$ the Lorenz-96
  estimator passes its own convergence check (median $11.9$); its earlier short-window failure was this same
  window-dependence (disclosed).
