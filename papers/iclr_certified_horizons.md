...
**(E12) The certificate's trustworthiness changes a budgeted decision.** On $40$-D Lorenz-96 an agent schedules sparse re-observations of a chaotic forecast under a fixed sensing budget. Holding the forecaster fixed (the $\mathbb{Z}_N$-equivariant model) and varying *only* the certificate that times re-observation: the equivariant certificate yields $8$–$16\%$ aggregate forecast-violation at the knee budget vs. $61$–$65\%$ for the non-equivariant certificate, whose $\lambda_1$ is inflated $\sim3\times$ and which therefore over-observes and starves the budget (margins $+0.45/+0.50/+0.57$, $3/3$). The two models forecast equally well (relMSE $\sim10^{-5}$); the gap is the certificate, which the equivariant model's spectral faithfulness (E2) makes calibrated. The value is an *a-priori* warm-start — a feedback-driven adaptive scheduler matches it only at $\sim3\times$ the budget; the contrast is within-method, so E11's $\sim2\times$ conservatism cancels. (A budget *allocation* across a chaoticity ensemble did *not* win — the learned spectrum is too inaccurate at low forcing to beat a flat allocation; reported honestly.) (`step85`, `step85b`.)
