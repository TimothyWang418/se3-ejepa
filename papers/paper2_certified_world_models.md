...
### 5.19 The certified horizon on a recognized chaotic control benchmark (Acrobot-v1) — accurate and binding for planning; the no-tuning return-win needs the predictive $\epsilon$ (Experiment 21)

**The certified horizon on a recognized chaotic control benchmark (Acrobot-v1) — accurate and binding for planning; the no-tuning return-win needs the predictive $\epsilon$ (Experiment 21).** Every prior horizon experiment lived on a *named dynamical system* (cat map, Hénon, Lorenz, Lorenz-96) or a robotics anchor we built; none was a *recognized RL control benchmark* where a horizon is supposed to *buy* something — a plan. Experiment 21 closes that gap on **Gymnasium Acrobot-v1**, the underactuated double-pendulum swing-up, with a learned action-conditioned $\mathbb{Z}_2$-equivariant (reflection frame-averaged) world model ($3$ seeds). The benchmark is genuinely chaotic — the true swing-up has finite-time $\lambda_1=0.094$ — and the learned model is a *good* one: one-step rollout relMSE $=4.8\times10^{-5}$. The cone certificate, as on every black-box net here, finds its continuum bridge vacuous (net-Jacobian Lipschitz $L_{J,\mathrm{net}}=21.7$) and **abstains to the bootstrap route** — the safe move per §5.18. Two of the three triad legs then land cleanly on this benchmark. **(i) Accuracy — the certified horizon tracks the measured one, in the two-regime $\epsilon$ pattern the whole paper predicts.** The certified $T_1(\epsilon)$ from the learned model and the measured rollout-divergence horizon agree better as $\epsilon$ coarsens: at the tight $\epsilon=0.01$, certified $T_1=312$ vs. measured $130$ (ratio $0.42$, the Proposition 8 finite-horizon optimistic regime); at $\epsilon=0.1$, $156$ vs. $73$ ($0.47$); at the asymptotic $\epsilon=0.3$, $82$ vs. $76$ (ratio $\mathbf{0.93}$, **predictive**). **(iii) Binding — the horizon controls the only thing that matters downstream, plan depth.** Sweeping a CEM/MPPI planner's plan depth $H$, task return has a *sharp interior optimum*: success $0.33$ at $H=1$, $\mathbf{1.00}$ at $H=78$ (return $-229$), and $\mathbf{0.00}$ at $H\ge156$ (return $-500$, total failure). Planning *past* the predictability horizon optimizes a rollout that has already diverged from reality, so the controller confidently executes a fantasy and fails; the binding gate is satisfied ($G$-binding TRUE, $H^\star=78$).

The centerpiece is a **three-way coincidence** that is not a coincidence: the optimal plan depth $H^\star\approx78$, the *measured* predictability horizon $76$ (at $\epsilon=0.3$), and the *certified* horizon $T_1=82$ (the predictive $\epsilon=0.3$, read a priori from the model's Jacobian field with no access to the true dynamics) all coincide. The best depth at which to plan **is** the model's predictability horizon — and the certificate names that depth in advance. This is the paper's thesis made operational on a benchmark: a horizon is not a diagnostic curiosity, it is the planning budget, and structure earns it a priori. We are deliberate about what does *not* yet hold. **Leg (ii) — the clean *no-tuning return-win* — is INCONCLUSIVE here**, and we report it as such rather than dress it up. The $G$-ii gate capped plan depth at the certified horizon computed at a *tight* $\epsilon=0.1$, which gives $T_1=156$ — roughly $2\times$ too deep (squarely the Proposition 8 optimistic regime) — so the certificate-aware planner at $H=156$ fails ($-500$) against a best-blind $H=78$ ($-229$). Capping at $T_1$ wins **only** at the *predictive* $\epsilon=0.3$ ($T_1=82\approx H^\star$); the naive "plan to $T_1$" rule overshoots at a tight $\epsilon$. So the honest verdict is two-of-three: accuracy and binding land on a recognized chaotic control benchmark, while the no-tuning return-win needs the predictive $\epsilon$ and is not claimed here. (A separate re-run at the predictive $\epsilon$ may later upgrade leg (ii) to a clean win; we do not bank on it now.) `experiments/step84`, `tests/test_step84.py`. (figure: `step84_certified_control_benchmark`, pending sync).
