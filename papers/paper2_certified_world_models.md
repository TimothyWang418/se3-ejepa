...
where the non-equivariant model's $\sim3\times$-inflated certificate over-observes and starves it ($8$–$16\%$ vs
...
**This is a structural phase transition, not a single-$N$ artifact (Step 83).** Sweeping $N\in\{12,20,28,40\}$ under the identical recipe, the $\mathbb{Z}_N$-conv holds full-spectrum $R^2>0.97$ throughout, while the dense MLP and the GRU are *statistically tied with it through $N{=}28$* and then **collapse at $N{=}40$** (MLP $R^2{=}-1.8$, GRU $R^2{=}-0.27$, $3$ seeds each): the unstructured models begin hallucinating spurious positive exponents (GRU $\sim\!31$ vs. the true $\sim\!14$) exactly where the dimension outruns their unstructured Jacobian. The single-$N{=}40$ separation is the *endpoint* of a clean structural phase transition.

![Experiment 18 — the structure-vs-scale-and-recurrence phase transition (Step 83). Full-spectrum Lyapunov $R^2$ vs. system dimension $N$ for the three architectures under the identical recipe ($3$ seeds/cell): the $\mathbb{Z}_N$-equivariant conv (blue) holds $R^2>0.97$ across $N$, while the dense MLP (red) and GRU-BPTT (green) are tied with it through $N{=}28$ and collapse at $N{=}40$. The single-$N$ separation is the endpoint of a structural phase transition.](figures/step83_rsquared_crossover.png)
...
**The certificate reads a public pretrained world model, training-free (Experiment 23).** Every certificate so far was read off a model *we* trained. Experiment 23 applies the unchanged machinery to the official **TD-MPC2** checkpoints (Hansen et al.; 5M single-task DMC models, MIT, 3 official seeds per task): rebuild the `_encoder/_dynamics/_pi` slices as line-faithful replicas, wrap the autonomous policy-prior loop $g(z)=d\big(z,\tanh\mu_\pi(z)\big)$ on the $512$-d SimNorm latent, and run the step78 Benettin-QR/bootstrap read-out — **no training, and no environment access on the certified side**. On **walker-walk** the latent loop is expansive ($\lambda_1=0.25$–$0.30$, tight CIs, $3/3$ seeds) and the paper's two-regime pattern reproduces on a model we did not train: optimistic at tight $\epsilon$ (measured/certified $0.08$–$0.20$ at $\epsilon{=}0.05$, the Proposition 8 $\delta$-bias), **calibrated at the coarse resolution** — ratio $\mathbf{0.94/0.95/1.02}$ at $\epsilon{=}0.2$, $3/3$ seeds, where the measured side open-loops the latent against the encoder run along the true dm\_control rollout. On **acrobot-swingup** the SimNorm-compact loop is *contracting* ($\lambda_1\approx-0.04$, $3/3$) and the certificate **abstains — correctly, by Proposition 7's scope theorem**: the residual divergence there (median $14$–$45$ steps) is bias-driven, not amplification-driven, and a Lyapunov certificate that issued a horizon anyway would over-claim. (Contrast our from-scratch Gymnasium-Acrobot model of §5.19, whose latent loop *is* expansive — different model, different latent geometry, and the certificate tracks each.) The $\sim\!64$ structural SimNorm zero-directions appear as a strongly-negative spectral band ($128$–$270$ directions; reported, not hidden); the certified scope is the policy-prior loop, not the MPPI planner. Closest prior work computes Lyapunov exponents of the *true environment* under RL policies (arXiv:2410.10674) or latent spectra for scientific-ML autoencoders (Özalp & Magri 2025); a Jacobian certificate of the *learned latent map* of a public RL world model, cross-validated against its true-environment divergence, is — to our knowledge — new. `experiments/step89`, `tests/test_step89.py`; checkpoints by URL, not vendored.
